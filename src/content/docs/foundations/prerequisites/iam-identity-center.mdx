...
You need one-time root (or admin) access to enable IAM Identity Center; MFA on the root user is recommended (see [AWS Account](/foundations/prerequisites/aws-account/)).

<Aside type="tip">
  If you prefer a plain IAM user with access keys, you can skip this page and continue with [AWS
  CLI](/foundations/prerequisites/aws-cli/).
</Aside>
...
1.  **Enable IAM Identity Center.** <Badge text="Optional" variant="note" size="small" />
    ...
    - Choose the same region you selected in the [AWS Account](/foundations/prerequisites/aws-account/) step. IAM Identity Center is a regional service; the region you enable it in is where the service runs
...
7.  **Configure the AWS CLI to use IAM Identity Center.** <Badge text="Required if using SSO" variant="tip" size="small" />
    ...
    Requires **AWS CLI v2** (see [AWS CLI](/foundations/prerequisites/aws-cli/) to install). After the CLI is installed, run:
...
Continue to [AWS CLI](/foundations/prerequisites/aws-cli/) to install the CLI and confirm your credentials work.
