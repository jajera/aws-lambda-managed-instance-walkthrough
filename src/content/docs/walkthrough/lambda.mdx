...
<Aside type="note">
  Terraform code: [terraform-aws-lambda-managed-instance](https://github.com/jajera/terraform-aws-lambda-managed-instance). This lab follows **`examples/waf-loki/`**, which wires **`module "lambda_managed_function_waf"`** (Node.js 24) with S3 read, **`LOKI_URL`**, and event sources **outside** the module (S3 notification and **`aws_lambda_permission`**). The sections below use **illustrative** snippets for the module shape; the authoritative input list is [modules/lambda_managed_function variables](https://github.com/jajera/terraform-aws-lambda-managed-instance/blob/main/modules/lambda_managed_function/variables.tf).
</Aside>
...
<Aside type="caution">
  `capacity_provider_config` cannot be added or removed after the function is first created. **`per_execution_environment_max_concurrency`** is also **immutable** after first create. Plan both before the first `terraform apply` that creates the Lambda.
</Aside>
