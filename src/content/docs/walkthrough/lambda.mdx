...
<Checklist
  ...
  items={[
    "[LMI stack](/walkthrough/lmi/) applied — `capacity_provider_arn` output is available",
    "Read [Publishing & activation](/overview/concepts/version-publishing/) — `capacity_provider_config` and `per_execution_environment_max_concurrency` are immutable after first create",
...
The [LMI stack lab](/walkthrough/lmi/) provisions the capacity provider. This module builds the **function side**: the execution IAM role, a pre-created CloudWatch log group, and a **published** `aws_lambda_function` with `capacity_provider_config` pointing at the provider ARN.
...
Further reading: [IAM roles](/overview/concepts/operator-role/) (execution vs operator).
...
Further reading: [Concurrency & runtimes](/overview/concepts/execution-environment/).
...
The module offers a **`cloudwatch_log_group_prevent_destroy`** flag; when `true`, `lifecycle.prevent_destroy` is set so `terraform destroy` cannot delete the log group. Further reading: [Monitoring](/overview/concepts/monitoring/).
...
Further reading: [IAM roles](/overview/concepts/operator-role/).
...
2. **Compare** your intended **`memory_size`**, **`per_execution_environment_max_concurrency`**, and capacity provider settings with [Publishing & activation](/overview/concepts/version-publishing/) before **`terraform apply`**.
...
For metrics, filters, and alarms, see **[Monitoring](/overview/concepts/monitoring/)**; for how publishes relate to capacity, see **[Publishing & activation](/overview/concepts/version-publishing/)**.

That finishes the **current** lab sequence: **VPC and networking**, **LMI stack**, then **Lambda Managed Function**. Concept pages under **[LMI fundamentals](/overview/concepts/)** remain the place for placement, scaling, and trade-offs. Further procedural labs will appear in the sidebar when those pages are enabled.
