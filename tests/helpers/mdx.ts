...
export function mdxPathForSlug(slug: string): string | null {
  const direct = join(DOCS_DIR, `${slug}.mdx`);
  if (existsSync(direct)) return direct;
  const index = join(DOCS_DIR, slug, "index.mdx");
  if (existsSync(index)) return index;
  return null;
}

export function slugFileExists(slug: string): boolean {
  return mdxPathForSlug(slug) !== null;
}

export function readMdxBySlug(slug: string): string {
  const path = mdxPathForSlug(slug);
  if (!path) {
    throw new Error(`No MDX file for slug: ${slug}`);
  }
